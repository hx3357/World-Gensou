...
public class Oscillator : MonoBehaviour
{
    ...
    [Range(0, 0.3f)] public double gain;
    [Range(20, 20000)] public double frequency = 440;
    ...
    [Range(0, 1)] public float lerpParam = 1;
    ...
    {
        if (!isEnabled) return;
        var w = 2 * Mathf.PI * (float)frequency / SAMPLE_RATE;
        for (var i = 0; i < data.Length; i += channels)
        {
            if (isSimpleLerp)
                data[i] = (float)gain * Mathf.Lerp(GetWaveTableValue(WaveType.Sawtooth, (float)phase),
                    GetWaveTableValue(WaveType.Square, (float)phase), lerpParam);
            else
