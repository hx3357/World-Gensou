...
public class GPUTrilinearScalerFieldDownSampler : IScalerFieldDownSampler
{
    ...
    private void InitBuffer()
    {
        ...
        newDotFieldBuffer =
            new ComputeBuffer(newDotFieldCount.x * newDotFieldCount.y * newDotFieldCount.z, sizeof(float) * 4);
    }
    ...
    private void RunDownSampleComputeShader()
    {
        var kernel = 0;
        ...
        out Vector3Int m_newDotFieldSize, out Vector3Int m_newChunkSize, out Vector3 m_newCellSize)
    {
        ...
        m_newDotFieldSize = newDotFieldCount = new Vector3Int((int)(m_dotFieldCount.x / downSampleRate),
            (int)(m_dotFieldCount.y / downSampleRate),
            (int)(m_dotFieldCount.z / downSampleRate));
        m_newChunkSize = new Vector3Int(newDotFieldCount.x - 1, newDotFieldCount.y - 1, newDotFieldCount.z - 1);
        m_newCellSize = newCellSize = new Vector3((m_dotFieldCount.x - 1) / (float)m_newChunkSize.x * cellSize.x,
    ...
    public bool GetState()
    {
        ...
        if (request.done) isDownSampled = true;
        return request.done;
    }
