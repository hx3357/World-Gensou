...
DotExpl octave_sdf(float3 pos, float baseRadius, float height)
{
    ...
    return sdf_cylinder(pos, baseRadius * (0.5 * get_hash(id) + 0.6), height, float3(1, 1, 1));
}
...
DotExpl fbm_shape_sdf(float3 pos, float3 origin, float baseRadius, float height, int octaves, float lacunarity,
                      float gain)
{
...
float neo_fbm_shape_sdf(float3 pos, float3 hash, float baseRadius, float height, int octaves, float lacunarity,
                        float gain)
{
    ...
    for (int i = 0; i < octaves; i++)
    {
        ...
        for (int j = 0; j < c; j++)
        {
            float theta = get_hash(hash + 58.6 * float3(i, j, 0)) * 2 * PI;
...
DotExpl debug_island_cluster_sdf(float3 pos, float3 origin, int iterateCount, float baseRadius, float height)
{
    ...
    {
        const float nextAngle = lastAngle - PI / 2 + get_hash(origin + float3(i, 0, 46.9)) * PI;
        const float nextDistance = baseRadius * (1 * get_hash(origin + float3(i, 34.8, 33.2)) + 2);
