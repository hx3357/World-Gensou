...
{
    private int[] surroundBox;
    public Vector3 center;
    public Vector3 size;
    public float volume;

    public static readonly SurroundBox InfiniteSurroundBox =
        new(int.MinValue, int.MaxValue, int.MinValue, int.MaxValue, int.MinValue, int.MaxValue);
    ...
    public bool IsInSurroundBox(Vector3Int chunkCoord)
    {
        if (surroundBox.Length != 6)
            throw new Exception("The surround box should have 6 elements");
    ...
    {
        var chunkCoord = Chunk.GetChunkCoordByPosition(worldPosition);
        return IsInSurroundBox(chunkCoord);
    }

    private int[] GetSurroundBox(Vector3 worldPosition, Vector3 worldSize)
    {
        var halfSize = worldSize / 2;
        var minChunkCoord = Chunk.GetChunkCoordByPosition(worldPosition - halfSize);
        var maxChunkCoord = Chunk.GetChunkCoordByPosition(worldPosition + halfSize);
        return new[]
        {
