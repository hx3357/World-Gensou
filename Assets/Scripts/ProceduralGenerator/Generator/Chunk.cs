...
{
    public static readonly Dictionary<Vector3, Chunk> zombieChunkDict = new();
    ...
    [Header("Debug")] public bool showMeshNormal = false;
    public float normalLength = 1f;

    private Mesh mesh;
    // Mesh cache
    private Dictionary<int, Mesh> lodMeshDict = new();
    private MeshFilter meshFilter;
    private MeshRenderer meshRenderer;
    private Vector3 volumeSize;
    private static float universalChunkSize;

    private static readonly float ZOMBIE_CHUNK_TIME = 15;
    ...
    {
        var origin = GetChunkOriginByCoord(coord);
        var volumeSize = universalChunkSize * Vector3.one;
    ...
    {
        origin = m_origin;
        center = origin + new Vector3((m_cellCounts.x - 1) * m_cellSize.x / 2, (m_cellCounts.y - 1) * m_cellSize.y / 2,
            (m_cellCounts.z - 1) * m_cellSize.z / 2);
    ...
    {
        if (zombieChunkDict.TryGetValue(origin, out var chunk)) zombieChunkDict.Remove(origin);
        Destroy(gameObject);
    }
    ...
    private void Update()
    {
        ...
        if (isZombie)
        {
            ...
            if (zombieTimer >= ZOMBIE_CHUNK_TIME) DestoryChunk();
        }
    }

    private void OnDrawGizmos()
    {
        if (isShowVolumeGizmo) Gizmos.DrawWireCube(transform.position + volumeSize / 2, volumeSize);

        if (isShowDotFieldGizmo) ShowDotFieldGizmo();
    }
}