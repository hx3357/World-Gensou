...
public class SDFIslandScalerFieldGenerator : GPUScalerFieldGenerator
{
    ...
    protected override void InitBuffer()
    {
        ...
        var isConcreteFlagBuffer = new ComputeBuffer(1, sizeof(int));
        isConcreteFlagBuffer.SetData(new[] { 1 });
        buffers.Add(isConcreteFlagBuffer);
        var isAirFlagBuffer = new ComputeBuffer(1, sizeof(int));
    ...
    protected override void GenerateRequest(ScalerFieldRequestData scalerFieldRequestData)
    {
        ...
        var isConcreteFlagBufferRequest = AsyncGPUReadback.Request(scalerFieldRequestData.buffers[1],
            sizeof(int), 0);
        requests.Add(isConcreteFlagBufferRequest);
        var isAirFlagBufferRequest = AsyncGPUReadback.Request(scalerFieldRequestData.buffers[2],
            sizeof(int), 0);
        requests.Add(isAirFlagBufferRequest);
    }

    protected override void SetComputeShaderParameters(ComputeShader m_cs,
        ScalerFieldRequestData scalerFieldRequestData, object[] parameters)
    {
        m_cs.SetFloat(IsoLevel, isoLevel);
        m_cs.SetVector(RandomOffset, randomOffset);
        if (parameters is { Length: > 0 })
        {
            var sfgParameter = new SDFIslandSFGParameter();
    ...
    public override (bool, Dot[], bool) GetState(ref ScalerFieldRequestData scalerFieldRequestData,
        bool isNotGetDotfield = false)
    {
        var dotFieldRequest = scalerFieldRequestData.requests[0];
        var isConcreteFlagBufferRequest = scalerFieldRequestData.requests[1];
        var isAirFlagBufferRequest = scalerFieldRequestData.requests[2];
        Dot[] dotField = null;
        scalerFieldRequestData.resultStates ??= new List<object>(new object[2]);
        scalerFieldRequestData.isDoneFlags ??= new bool[3];
        if (isConcreteFlagBufferRequest.done && !scalerFieldRequestData.isDoneFlags[0])
        {
            var isConcreteFlag = new int[1];
        ...
        {
            if (!isNotGetDotfield) dotField = dotFieldRequest.GetData<Dot>().ToArray();
            scalerFieldRequestData.isDoneFlags[2] = true;
        }

        var isDone = isConcreteFlagBufferRequest.done && isAirFlagBufferRequest.done && dotFieldRequest.done;
