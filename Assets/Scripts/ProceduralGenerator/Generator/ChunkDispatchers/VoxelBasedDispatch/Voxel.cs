...
namespace ChunkDispatchers.VoxelBasedDispatch
{
    public class Voxel
    {
            ...
            , Action<Voxel> onGenerate = null)
        {
            ...
            dotCount = m_isRoot ? PoissonSampler.GetPoissonSampleCount(initDotExp, center) : m_dotCountExpection;
        ...
        private void Spilt(int curDepth, float dotCountExp, Action<Voxel> onGenerate)
        {
            if (curDepth >= MAX_DEPTH)
                return;


            var subVoxelDotCountList = new int[8];
            var newOriginList = new Vector3[8];
            var isFilledList = new bool[8];
            var subVoxelBitmap = 0;
            var subVoxelCount = 0;

            for (var i = 0; i < 8; i++)
            {
                var newOrigin = worldOrigin + new Vector3(i & 1, (i & 2) >> 1, (i & 4) >> 2) * worldSize / 2;
                var subVoxelDotCount =
            ...
            if (subVoxelCount > 1)
            {
                ...
                var displacements = VoxelDisplacementTable.Table[subVoxelBitmap];
                var displacement =
                    displacements[Mathf.Abs((center * 0.127f + 57 * Vector3.one).GetHashCode()) % displacements.Length];
                ...
                var finalDisplacementCenterMap = new Vector3[8];

                foreach (var kvPair in subVoxelDisplacedCentersDict)
                {
                    var displacementAreaOrigin = int.MaxValue * Vector3.one;
                    var displacementAreaEnd = -displacementAreaOrigin;
                    ...
                    finalDisplacementCenterMap[kvPair.Key] = displacementAreaOrigin +
                                                             Vector3.Scale(
                                                                 HashUtility.Get3DHash(newOriginList[kvPair.Key]),
                ...
                for (var i = 0; i < 8; i++)
                {
                    if (!isFilledList[i])
                        continue;
                    var finalOrigin = finalDisplacementCenterMap[i];
                    var finalSize = worldSize / 2;

                    //Make the sub voxel close together
                    finalSize *= 1 + subVoxelExpandFactor;
                    finalOrigin += (worldOrigin - finalOrigin).normalized *
                                   ((finalSize - worldSize / 2) / 2 * 1.42f * 1.2f);
        ...
        public void CalculateChunkCoords(Dictionary<Vector3Int, ChunkParameter> chunkCoordMap, bool isForce = false)
        {
            ...
            if (isLeaf)
            {
                var chunkCoords = GetChunkCoords();
            ...
            else
            {
                ...
                foreach (var childVoxel in childVoxels) childVoxel?.CalculateChunkCoords(chunkCoordMap);
            }
        }
        ...
        public void RemoveChunkCoords(Dictionary<Vector3Int, ChunkParameter> chunkCoordMap)
        {
            ...
            foreach (var chunkCoord in curChunkCoordMap.Keys) chunkCoordMap.Remove(chunkCoord);
        }
        ...
        public float GetMaxDistance(Vector3 pos)
        {
            ...
            {
                var distance = Vector3.Distance(corner, pos);
        ...
        private Vector3Int ConvertToChunkCoord()
        {
        ...
        {
            var voxelOrigin = worldOrigin;
            var voxelEnd = worldOrigin + Vector3.one * worldSize;
            var startChunkCoord = Chunk.GetChunkCoordByPosition(voxelOrigin);
            var endChunkCoord = Chunk.GetChunkCoordByPosition(voxelEnd);
            if (Vector3.Distance(voxelEnd, Chunk.GetChunkOriginByCoord(endChunkCoord)) < 0.01)
                endChunkCoord -= Vector3Int.one;
            var chunkCoords = new List<Vector3Int>();
            for (var x = startChunkCoord.x; x <= endChunkCoord.x; x++)
            for (var y = startChunkCoord.y; y <= endChunkCoord.y; y++)
            for (var z = startChunkCoord.z; z <= endChunkCoord.z; z++)
        ...
        public Voxel[] GetLeafVoxels()
        {
            ...
            var leafVoxels = new List<Voxel>();
            foreach (var childVoxel in childVoxels) leafVoxels.AddRange(childVoxel.GetLeafVoxels());
