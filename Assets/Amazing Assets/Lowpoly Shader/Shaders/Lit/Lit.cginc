...
#if defined(_LOWPOLY_UNLIT)
void surf (v2f_surf IN, out float3 outAlbedo, out float outAlpha, out float3 outEmission)
#else
...
#endif
{
        ...
        float3 alpha = float3(1, mainColor.a, 0);
            ...
            outAlbedo = mainColor.rgb;

            #if defined(LOWPOLY_SHADER_TYPE_TRANSAPRENT)
                outAlpha = alpha[_Lowpoly_TransparentFade + 1];
            #else
                outAlpha = 1;
            #endif

            outEmission = wireframeEmission;
